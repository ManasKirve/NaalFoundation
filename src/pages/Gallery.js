...
const Gallery = () => {
  ...
  const filteredItems = filter === 'Education'
    ? galleryItems
        ...
        <div className="container">
          <div className="row justify-content-center text-center">
            <div className="col-md-8">
              <h1>Our Gallery</h1>
              <p className="lead">Inspiring moments from our journey of creating positive change</p>
            </div>
          </div>
        </div>
      </section>
...
<div>
         <button
           ...
           <button
            ...
            <button
              className={`filter-btn ${filter === 'Agriculture' ? 'active' : ''}`}
...
<div className="col-md-7">
  {filter === 'education' && (
    <div className="">
      ...
      <p>We support bright and deserving students by providing scholarships and resources, helping them pursue their dreams without financial barriers. Our goal is to nurture knowledge, confidence, and opportunities for a better future.</p>
    </div>
  )}
  ...
  {filter === 'Agriculture' && (
    <div className="">
      ...
      <p>We extend financial support to farmers in need, helping them overcome challenges and sustain their livelihoods. Our focus is on empowering the farming community and strengthening the backbone of our village economy.</p>
    </div>
  )}
  ...
  {filter === 'Healthcare' && (
    <div className="">
      ...
      <p>We are committed to improving community health by providing essential services such as ambulance support and access to clean drinking water. Our efforts focus on ensuring timely medical help and promoting overall well-being.</p>
    </div>
  )}
  ...
  {filter === 'Environment' && (
    <div className="">
      ...
      <p>We actively engage in tree plantation drives to create a greener tomorrow and raise awareness about sustainability. Our efforts aim to protect nature and promote a healthier environment for future generations.</p>
    </div>
  )}
</div>
          ...
          <div className="row g-4">
            {filteredItems.map(item => (
              <div className="col-md-4" key={item.id}>
                <div className="gallery-item" onClick={() => handleImageClick(item)}>
                  <img src={item.image} alt={item.title} className="img-fluid border-image" />
      ...
      {/* Image Modal */}
      <div className={`modal fade ${currentImage ? 'show' : ''}`} id="imageModal" tabIndex="-1" style={{ display: currentImage ? 'block' : 'none' }}>
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">

              <button type="button" className="btn-close" onClick={() => setCurrentImage(null)}></button>
            </div>
            <div className="modal-body">
              <img src={currentImage?.image} alt={currentImage?.title} className="img-fluid" />
